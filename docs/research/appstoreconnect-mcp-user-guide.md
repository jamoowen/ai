...
- The checked-in `.gitignore` currently does **not** include `*.p8`. The guide must not imply that the repository automatically protects a key file; a separate implementation decision is whether to add that ignore rule.
